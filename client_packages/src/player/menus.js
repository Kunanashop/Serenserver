...
mp.events.add('client::closenavigator', () => {
    global.navigatorMenu.active = false
	global.navigatorMenu.execute('navigatorMenu.style=0');
    global.navigatorMenu = null;
	mp.gui.cursor.visible = false;
});
mp.events.add('open_navigator', () => {
    if (global.navigatorMenu == null) {
		mp.gui.cursor.visible = true;
        global.navigatorMenu = mp.browsers.new("http://package/browser/modules/Navigator/index.html")
		global.navigatorMenu.execute(`navigatorMenu.set(${selectedDictorYandexIndex})`)
    } else if (global.navigatorMenu.active == false){
        mp.gui.cursor.visible = true;
		global.navigatorMenu.active = true
		global.navigatorMenu.execute(`navigatorMenu.set(${selectedDictorYandexIndex})`)
    }
});
