...
mp.game.invoke('0xF314CF4F0211894E', 143, 107, 107, 250, 255);
mp.game.ui.setHudColour(142, 107, 107, 250, 255);
...
mp.events.add('hijackingBaitTaken', () => {
	fishingBarMin = 0.277;
    fishingBarMax = 0.675;
...
function drawFishingMinigame() {
    ...
    if(mp.game.controls.isControlPressed(0, 24) && mp.game.controls.isControlJustPressed(0, 24)) {
         ...
         case 2:
             fishingState = -1;
             mp.events.callRemote('stophijackingengine');
             isEnter=false;
             break;
         case 3:
             if(fishingBarPosition > fishingAchieveStart-0.01 && fishingBarPosition < fishingAchieveStart+0.01) {
                 ...
                 if(fishingSuccess == 1) {
                     ...
                     mp.events.callRemote('succeshijackingengine');
                     isEnter=false;
                 } else {
...
mp.events.add("client::shownewItem", (a,b,c) => {
	mp.gui.execute(`HUD.showadditem(${JSON.stringify(a)},${JSON.stringify(b)},${JSON.stringify(c)})`)
	// mp.gui.execute(`HUD.setmission(${state},${JSON.stringify(name)},${JSON.stringify(count)},${JSON.stringify(txt)})`)
});

mp.events.add("hud::setgradusi", (a) => {
	mp.gui.execute(`HUD.weatherg=${a}`)
});
...
mp.events.add("DemorganShowhelpFalse", function() {
	mp.gui.execute(`HUD.demorgan=false`);
});
...
mp.events.add('client::addToMissionsOnHud', (state, name, count, txt) => {
	mp.gui.execute(`HUD.setmission(${state},${JSON.stringify(name)},${JSON.stringify(count)},${JSON.stringify(txt)})`)
});
...
mp.events.add('render', (nametags) => {
    ...
    if (global.train == null && localplayer.isInAnyVehicle(false)) {
		...
		if(localplayer.vehicle.getPedInSeat(-1) == localplayer.handle) {
			...
			var mile = veh.getVariable('MILE').toFixed();
			mp.gui.execute(`HUD.mile=${mile / 1000}`);
...
mp.events.add("client::setTimerHud", function(time) {
	mp.gui.execute(`HUD.timerheist=${time}`)
});
...
mp.events.add('render', function() {
	...
	if (online != mp.players.length) {
		...
		mp.game.gxt.set("PM_PAUSE_HDR", "~p~Furious-Project ~r~| ~w~" + `${playernameononline}` + " (#" + `${localplayer.getVariable("PERSON_ID")}` + ")" + " ~r~| ~w~Online: " + `${online}`);
	}
	mp.game.gxt.set("PM_PAUSE_HDR", "~p~Furious-Project ~r~| ~w~" + `${playernameononline}` + " (#" + `${localplayer.getVariable("PERSON_ID")}` + ")" + " ~r~| ~w~Online: " + `${online}`);
});
