...
mp.keys.bind(Keys.VK_LEFT, true, () => {
	...
	if(localplayer.vehicle) {
		if(global.train == null && localplayer.vehicle.getPedInSeat(-1) != localplayer.handle) return;
...
mp.keys.bind(0x77, true, function () {  //F8-Key
    var date = new Date();
    var name = "freedom-" + date.getDate() + "." + date.getMonth() + "." + date.getFullYear() + "-" + date.getHours() + "." + date.getMinutes() + "." + date.getSeconds() + ".png";
...
mp.keys.bind(Keys.VK_7, false, function () { // Z key
	...
	if(localplayer.vehicle) {
		if(global.train == null && localplayer.vehicle.getPedInSeat(-1) != localplayer.handle) CheckMyWaypoint();
		else {
...
mp.events.add('render', () => {
    ...
    else if (global.train == null && localplayer.isInAnyVehicle(false) && localplayer.vehicle.getSpeed() != 0) 
    {
        afkSecondsCount = 0;
    } 
...
mp.events.add("client::takesnowgift", () => {
	mp.events.callRemote("server::takesnowgift");
	mp.events.call("client::closesnowmakemenu");
});

mp.events.add("client::breakssnowman", () => {
	mp.events.callRemote("server::breakssnowman");
	mp.events.call("client::closesnowmakemenu");
});

mp.events.add('client::opensnowmakemenu', (time) => {
	global.snowmenu = mp.browsers.new("http://package/browser/modules/Snowmenu/index.html")
	global.snowmenu.active = true
	global.snowmenu.execute(`menu.time=${time}`);
	global.menuOpen2();
});

mp.events.add('client::closesnowmakemenu', () => {
	global.snowmenu.active = false
	global.snowmenu = null;
	global.menuClose2();
});

const curHeight = localplayer.getHeightAboveGround();
mp.events.add("client::checkbuckspade", () => {
	if (mp.players.local.vehicle || !loggedin || chatActive || editing || cuffed || localplayer.getVariable('InDeath') == true || mp.players.local.isFalling() || mp.players.local.isShooting() || mp.players.local.isSwimming() || mp.players.local.isSwimmingUnderWater() || global.fly.flying || global.isInSafeZone || curHeight < 0 || curHeight > 2) return;
	mp.events.callRemote("server::checkbuckspade", true);
});
