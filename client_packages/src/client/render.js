...
function getLookingAtEntity() {
    ...
    if (typeof result !== 'undefined') {
		...
		if (result.entity.type == 'ped') return;
...
mp.keys.bind(0x47, false, function () { // G Menu Veh
    if (global.menuCheck() || localplayer.getVariable('InDeath') == true && !global.localplayer.isInAnyVehicle(false)) return;
    if (circleOpen) {
        CloseCircle();
        return;
    }
    ...
    switch (entity.type) {
        ...
        case "player":
            ...
            global.soundCEF.execute(`playSounds('./sounds/open_g.mp3')`);
            break;
        case "vehicle":
