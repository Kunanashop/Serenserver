...
mp.keys.bind(0x60, true, _ => {
    if (global.localplayer.vehicle && global.localplayer.vehicle.getPedInSeat(-1) === global.localplayer.handle && global.localplayer.vehicle.getClass() === 18) {
        global.localplayer.vehicle.getVariable('silentMode') ? mp.events.call('notify', 2, 8, "Безшумный режим сирен выключен", 3000) : mp.events.call('notify', 2, 8, "Безшумный режим сирен включен", 3000);
        mp.events.callRemote('syncSirens', global.localplayer.vehicle)
    }
});
