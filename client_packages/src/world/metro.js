...
mp.events.add('createtrain', async (pos) => {
	global.train = mp.game.vehicle.createMissionTrain(25, pos.x, pos.y, pos.z, true);
	mp.game.invoke("0x16469284DB8C62B5", global.train, 0) 
	trainseat = true;
	await invehtrain();
	setTimeout( function() {
		mp.game.invoke("0x16469284DB8C62B5", global.train, 25.5) 
	}, 2000);
});

mp.events.add("playerQuit", (player) => {
  if (player == localplayer && global.train) {
    mp.game.vehicle.deleteMissionTrain(global.train);
	global.train = null;
...
async function invehtrain() {
  ...
  while (localplayer.isInAnyVehicle(false) == false || localplayer.getVehicleIsIn(false) != global.train) {
    localplayer.setIntoVehicle(global.train, 0);
...
mp.events.add('speedtrain', (count) => {
	mp.game.invoke("0x16469284DB8C62B5", global.train, count) 
});
