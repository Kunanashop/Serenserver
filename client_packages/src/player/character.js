...
mp.events.add('characterSave', (age) => {
	...
	if(global.editorBrowser != null) {
		global.editorBrowser.destroy();
		global.editorBrowser = null;
		let currentGender = (gender) ? 0 : 1;
...
function checkCamInAirCharSelectChar() {
    if (mp.game.invoke(Natives.IS_PLAYER_SWITCH_IN_PROGRESS)) {
       ...
       if(global.editorBrowser == null) global.editorBrowser = mp.browsers.new('http://package/browser/modules/Character/index.html#content-1');
		global.menuOpen();
		skycam = false;
    }
}
